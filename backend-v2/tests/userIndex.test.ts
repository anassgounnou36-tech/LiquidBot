...
describe('UserIndex', () => {
  ...
  it('should update user tokens and track users', () => {
    const user1 = '0xUser1';
    const token1 = '0xToken1';
    const token2 = '0xToken2';

    userIndex.updateUserTokens(user1, [token1, token2]);
  ...
  it('should normalize addresses to lowercase', () => {
    ...
    userIndex.updateUserTokens(user1, [token1]);
  ...
  it('should track multiple users for the same token', () => {
    ...
    userIndex.updateUserTokens(user1, [token1]);
    userIndex.updateUserTokens(user2, [token1]);
  ...
  it('should handle user with multiple tokens', () => {
    ...
    userIndex.updateUserTokens(user1, [token1, token2, token3]);
  ...
  it('should update user tokens when called multiple times for same user', () => {
    const user1 = '0xUser1';
    const token1 = '0xToken1';
    const token2 = '0xToken2';
    ...
    // Second update with additional token
    userIndex.updateUserTokens(user1, [token2]);
    expect(userIndex.getUsersForToken(token2).size).toBe(1);
